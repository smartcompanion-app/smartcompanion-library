...
The `es2015` and `es2017` fields are gone. They came from the original scaffold and pointed at `dist/esm/index.mjs`, which Stencil v4 does not emit — the package ships no `.mjs` at all. A field naming a missing file is worse than no field: the legacy toolchains that read them now fall through to `module`/`main`, which exist. Do not restore them.