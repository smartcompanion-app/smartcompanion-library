import { Asset, AssetRepository, Station } from "..";
import { Storage } from "../../storage";

export class StationRepository {

  protected storage: Storage;
  protected assetRepository: any;

  constructor(storage: Storage, assetRepository: AssetRepository) {
  ...
  async getStation(stationId: string): Promise<Station> {
    ...
    if (station && station.images) {
      for (let i = 0; i < station.images.length; i++) {
        station.images[i] = await this.assetRepository.getAsset(station.images[i] as string);
      }
    }

    if (station && station.audios) {
      for (let a = 0; a < station.audios.length; a++) {
        station.audios[a] = await this.assetRepository.getAsset(station.audios[a] as string);
      }
    }
