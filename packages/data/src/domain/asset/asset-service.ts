...
export class AssetService {
  ...
  // with internalUrl resolved
  async getAssets(filter: { language?: string; excludeNoLanguage?: boolean } = {}): Promise<Asset[]> {
  ...
  getUnresolvedAssets(filter: { language?: string; excludeNoLanguage?: boolean } = {}): Asset[] {
    ...
    for (const asset of assets) {
      if ((!!filter.language && !!asset.language && asset.language == filter.language) || (!filter.excludeNoLanguage && !asset.language)) {
        result.push(asset);
      }
    }
